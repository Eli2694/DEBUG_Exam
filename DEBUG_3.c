...
typedef struct letter {
	...
	char big;
	int counter;
	//BUG. to create linkedlist we need pointers next and prev
	struct letter* next;
	struct letter* prev;
}letter;
...
int main()
{
	...
	// BUG.lettersCountArr[20] => lettersCountArr[26] alphabets has 26 letters
	int lettersCountArr[26] = { 0 };
	char readChar, ch;
	FILE* f;
	// BUG. myFile.txt does not exist
	f = fopen("myFile.txt", "r");
	// Display the content of myFile.txt and count letters
	printf("the content of myFile.txt :\n");
	// BUG. need to replace 0 with EOF => !=EOF
	...
	for (int j = 0; j < strlen(smallAlpha); ++j)
	{
		...
		// BUG. for linkedlist we need HEAD (start of list)
		if (HEAD == NULL)
		{
